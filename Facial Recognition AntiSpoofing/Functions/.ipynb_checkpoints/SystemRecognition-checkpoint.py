...
# Show profile
def Profile():
    global step, count, sample, UserName, OutFolderPathUser
    # Variables reset
    step = 0
    count = 0
    ...
    background2 = Label(image=BG_Image2, text="Start")
    ...
    if User in classes:
        ...
        # Label Img
        labelimage = Label(screen4)
        labelimage.place(x=490,y=80)
...
def BiometricLog():
    global screen2, screen3, LogUser, LogPass, step, OutFolderPathFace, capt, VideoLabel, FaceCode, classes, images, blink, count
    # Check Capt
    if capt is not None:
        ret, frame = capt.read()
